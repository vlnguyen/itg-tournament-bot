...
export class ChartsController {
  ...
  /**
   * `PATCH /api/tournaments/:id/charts` — the pack management table's
   * Save. Unlike import, this is never gated by tournament state: "chart
   * edits [need] no freeze rule... a chart already drawn renders from its
   * own copy," DESIGN.md's "Snapshotting a chart." `updates`/`deletes` are
   * filtered down to chart ids this tournament actually owns before
   * anything runs, so a stale or forged id can't touch another
   * tournament's pack.
   */
  @Patch(':id/charts')
  async commitPackChanges(
    @Param('id') id: string,
    @Body() body: unknown,
    @CurrentUser() discordUserId: string | null,
    ...
    if (!discordUserId || !(await this.tierService.hasTier(tournament.guildId, discordUserId, Tier.TOURNAMENT_ORGANIZER))) {
      throw new ForbiddenException('You need Tournament Organizer tier to edit the pack.');
    }

    let request: CommitPackChangesRequest;
    try {
      request = CommitPackChangesRequest.parse(body);
    } catch (err) {
      if (err instanceof ZodError) throw new BadRequestException(err.issues);
      throw err;
    }

    const requestedIds = [...new Set([...request.updates.map((u) => u.chartId), ...request.deletes])];
    const owned = await this.prisma.chart.findMany({ where: { id: { in: requestedIds }, tournamentId: id }, select: { id: true } });
    const ownedIds = new Set(owned.map((c) => c.id));

    const deletes = request.deletes.filter((chartId) => ownedIds.has(chartId));
    const deleteSet = new Set(deletes);
    const updates = request.updates.filter((u) => ownedIds.has(u.chartId) && !deleteSet.has(u.chartId));

    await this.prisma.$transaction([
      ...updates.map(({ chartId, ...fields }) => this.prisma.chart.update({ where: { id: chartId }, data: fields })),
      ...(deletes.length > 0 ? [this.prisma.chart.deleteMany({ where: { id: { in: deletes } } })] : []),
    ]);
    await logAction(this.prisma, discordUserId, 'PACK_EDITED', 'Tournament', id, { updated: updates.length, deleted: deletes.length });

    return { updated: updates.length, deleted: deletes.length };
  }
}
