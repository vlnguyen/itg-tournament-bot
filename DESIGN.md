...
`Tournament.defaultFormatKey` is what a match is stamped with absent anything more specific. `Tournament.formatOverrides` — `Record<matchKey, FormatKey>` — is the TO's per-match intent, keyed by the same `matchKey(ref)` (`"WINNERS:4:0"`) the bracket UI already uses to address a cell. Stamping a generated match is `formatOverrides[matchKey(ref)] ?? defaultFormatKey`: mixing Bo3 through the early rounds with Bo5 for Winners Finals, Losers Finals, the Grand Final and its reset is a handful of per-match assignments, not a schema change or a migration — the reducer, the event log, replay, and every projection already work per match regardless of which format each one carries.
...
**Several things are checked at the start transition, and only the pack-size warning is allowed to merely warn.** Permissions are re-checked and a missing one blocks with the exact list. A bracket generated ahead of time (`GENERATE_BRACKET`, see "The format belongs to the match") must still match the surviving field — a mismatch blocks with a message naming both counts, rather than starting against a stale bracket. A song pack below the recommended size warns and proceeds — the requirement is explicit that the warning never blocks. The threshold is **`recommendedPackSize` from the format**, not a constant: Bo5 asks for 10, Bo3 asks for 8, and a tournament mixing them takes the maximum across every format actually assigned — the default plus every per-match override — not just the default's own threshold.

The start also asserts that **every format a generated match could carry resolves to a registered key** — the default and every value in `formatOverrides`, not only the default. A typo an override was set with is caught here as a refused start, rather than surfacing later as a match that cannot be played.
...
**Results go to their own channel, one embed per finished match** (`buildResultAnnouncement`, `render/result-summary.ts`), colored white. The title — `{round} — {Player A} vs {Player B}`, both names in seat order regardless of who won — is the whole embed's link target (`setURL`) to the match's page on the public bracket; Discord embed titles carry no inline markdown, so "the match name as a hyperlink" is necessarily the *entire* title linked, not a styled span within it. The description is `{winner} advances ({winner score}-{loser score})`, then — when at least one song was played — a blank line and a numbered line per song (`{n}. ` — `song.index + 1`, continuous across tiebreaks — then the bolded chart, or `Tiebreak {round} — {chart}` for a tiebreak round, then 🏆 winner or 🤝 `Tie`, no EX%, which stays on the in-thread result summary), then a blank line and the tournament name linked to its own page (`[{tournament name}](tournamentUrl)`, which description fields *do* render as a real link, unlike the title).
